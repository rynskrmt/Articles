...
WikiのコンテンツはMarkdownで管理され、ファイルシステムに保存されます。  
通常のWikiの機能は備えており、シンプルで使いやすいインターフェースを提供しています。  


## 導入・起動方法

Wiki-Goは、単一のバイナリでビルドされています。  
したがって、実行ファイルをダウンロード・実行すればとりあえず手元で動かせます。
...
通常通り、実行ファイルをダブルクリックで実行しても構いません。  
```terminal
./wiki-go-mac-arm64
```
...
個人的に、ユーザインターフェースの言語設定を日本語に変更可能なのはありがたいです。  
:::message
日本語に変更すると一部UIが崩れてしまう現象がありました。使用には全く問題ありません。
※もしかしたらOSS貢献のチャンスかもしれないので詳しく調査してみます。 　
:::
...
また、slug（コンテンツのURLの一部となる文字列）も設定できます。   
タイトル入力と同時にslugも入力される形式なのは地味にありがたい。    

:::message  
文書パスを日本語で登録しようとすると、ディレクトリ名が空欄（内部的には`---`）で表示されてしまいます。 　  
:::
...
「**Wiki-Go**」は、単一バイナリで動作するシンプルな**Flat-File Wiki**です。

- **動かすまでが非常に簡単**：実行ファイルをダウンロードして起動するだけ
- **データベース不要**：コンテンツはMarkdownファイルとしてファイルシステムに保存される
- **必要十分な機能**：権限設定、多言語対応、MathJax、Mermaid記法など、Wikiに必要な機能を備えている

ナレッジ共有ツールの導入を検討している方は、まず「**Wiki-Go**」を試してみてはいかがでしょうか。 　　


## 余談： 現場で「成立する」ソフトウェアとは

ソフトウェアは「作れる」だけでは不十分で、「使われる」ことが重要でありスタートラインです。 
...
ソフトウェアエンジニアとして、**現実の組織・制約・意思決定の中で、ソフトウェアを“成立させる”とは何か**を再考する良いきっかけになりました。  
この話題もまたどこかのタイミングで記事にしたいと思います。  
